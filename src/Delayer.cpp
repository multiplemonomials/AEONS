...
Delayer * Delayer::factory(float min_delay_in_ms, uint16_t total_delayer_calls)
{
	...
	//starts at 1
	uint16_t required_array_length = 1;
	...
	{//nessecary so that counter does not persist
	...
	//ORDER UP!
	DelayRun * delay_map = new DelayRun[required_array_length];
	...
	for(uint16_t counter = 1; min_delay <= delay_map[counter-1]._run_delay;)
	{
		// Calculate the delay
	...
	for(uint16_t counter = 1; last_unique_value - counter >= 0; counter ++)
	{
		...
		//it IS alwas good to have a copy constructor lying around.
	 ...
	 */
		...
		Serial.print(required_array_length);
...
void Delayer::operator ()()
{
	...
	// ASSERT(_call_count < _delay_map_length);
